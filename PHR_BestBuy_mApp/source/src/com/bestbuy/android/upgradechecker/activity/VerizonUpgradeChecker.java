...
public class VerizonUpgradeChecker extends MenuActivity implements OnLongClickListener{
	private EditText pinEditText, phoneNumberEditText, last4SSNEditText,zipCodeEditText;
	private String zip; 
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.upgradechecker_verizon);
		...
		pinEditText = (EditText) findViewById(R.id.pin_password_field);
		...
		pinEditText.setOnLongClickListener(this);		
		...
		continueBtn.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				...
				else if(pinEditText.getText().toString().trim().length() == 0){
					popupDialog("Pin/Password cannot be left empty");
				}
				else if(pinEditText.getText().toString().length()==0) {
					popupDialog("Please enter a valid Pin/Password field");
				}
				else {
					new UpgradeCheckerTask(VerizonUpgradeChecker.this).execute();
				} 	
			}
		});
		...
		phoneNumberEditText.setOnTouchListener(new PhoneNumberClearingTouch(phoneNumberEditText,closeImage,UpgradeCheckerUtils.VERIZON_CLEAR_PH_NO));
	...
	public void onResume(){
		...
		String cachedPhoneNumber = AppData.getSharedPreferences().getString(AppData.VERIZON_UPGRADE_CHECKER_PHONE_NUMBER, null);
	...
	private void popupDialog(String message){
		...
		alert.setNegativeButton("OK", new DialogInterface.OnClickListener(){
			public void onClick(DialogInterface dialog, int which) {
				if((phoneNumberEditText.getText().toString().trim().length()>0)&&(last4SSNEditText.getText().toString().trim().length()>0)&&(zipCodeEditText.getText().toString().trim().length()>0)&&(pinEditText.getText().toString().trim().length()>0))
				{
					if (!(phoneNumberEditText.getText().toString().trim().length() < 12)||!(last4SSNEditText.getText().toString().trim().length()<4)||!(zipCodeEditText.getText().toString().trim().length()<5)||!(pinEditText.getText().toString().trim().length()<=0))					
						clearInputField();
				}
			}
		});
	...
	class UpgradeCheckerTask extends BBYAsyncTask{
		...
		protected void onPreExecute() {
			super.onPreExecute();	
			application = getApp();
			application.setPrimaryPhoneNumber(phoneNumberEditText.getText()
					.toString());
			zip=zipCodeEditText.getText().toString();
			application.setZipCode(zip);
			application.setLast4SSN(last4SSNEditText.getText().toString());
			application.setPin(pinEditText.getText().toString());
			application.setCarrierCode("VERIZON");
			service = new UpgradeEligibilityService(VerizonUpgradeChecker.this);			
		...
		public void doFinish() {
			UpgradeChecker.SELECTED_CARRIER="VERIZON";
			HashMap results = getApp().getResults();
			if (results != null && results.get("error") != null
					&& ((String) results.get("error")).length() > 0) {			

				popupDialog("Please double-check your account info and try again!");
				return;
			}
			UpgradeCheckerDataLogic dataLogic=new UpgradeCheckerDataLogic(VerizonUpgradeChecker.this);
			List<Subscriber> subscriberList=dataLogic.getSubcriberList(zip);
			getApp().setSubscribers(subscriberList);
			clearFieldOnResume = true;
			AppData.getSharedPreferences().edit().putString(AppData.VERIZON_UPGRADE_CHECKER_PHONE_NUMBER, phoneNumberEditText.getText().toString()).commit();
			if(subscriberList.size()==1){
				Intent intent = new Intent(VerizonUpgradeChecker.this, UpgradeCheckerTermsandConditions.class);
				intent.putExtra("Subscriber", subscriberList.get(0));
				startActivity(intent);
			}
			else{
				Intent intent = new Intent(VerizonUpgradeChecker.this, PhoneNumberListActivity.class);		
				startActivity(intent);
			}
		}
		...
		public void doError() {
			NoConnectivityExtension.noConnectivity(activity, new OnReconnect() {
				public void onReconnect() {
					new UpgradeCheckerTask(VerizonUpgradeChecker.this)
							.execute();
				}
			}, new OnCancel() {
