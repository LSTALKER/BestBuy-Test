...
public class SprintUpgradeChecker extends MenuActivity implements OnLongClickListener {
	private String zip; 
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.upgradechecker_sprint);
		...
		continueBtn.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {								
				...
				if(phoneNumberEditText.getText().toString().trim().length() == 0){
					...
					new UpgradeCheckerTask(SprintUpgradeChecker.this).execute();
				} 
			}
		});
		...
		phoneNumberEditText.setOnTouchListener(new PhoneNumberClearingTouch(phoneNumberEditText,closeImage,UpgradeCheckerUtils.SPRINT_CLEAR_PH_NO));
	...
	public void onResume(){
		...
		String cachedPhoneNumber = AppData.getSharedPreferences().getString(AppData.SPRINT_UPGRADE_CHECKER_PHONE_NUMBER, null);
	...
	class UpgradeCheckerTask extends BBYAsyncTask{
		...
		protected void onPreExecute() {
			...
			application.setCarrierCode("SPRINT");
			service = new UpgradeEligibilityService(SprintUpgradeChecker.this);			    		
		...
		public void doFinish() {
			UpgradeChecker.SELECTED_CARRIER="SPRINT";
			...
			UpgradeCheckerDataLogic dataLogic=new UpgradeCheckerDataLogic(SprintUpgradeChecker.this);
			List<Subscriber> subscriberList=dataLogic.getSubcriberList(zip);
			getApp().setSubscribers(subscriberList);
			clearFieldOnResume = true;
			AppData.getSharedPreferences().edit().putString(AppData.SPRINT_UPGRADE_CHECKER_PHONE_NUMBER, phoneNumberEditText.getText().toString()).commit();
			if(subscriberList.size()==1){
				Intent intent = new Intent(SprintUpgradeChecker.this, UpgradeCheckerTermsandConditions.class);
				intent.putExtra("Subscriber", subscriberList.get(0));
				startActivity(intent);
			}
			else{
				Intent intent = new Intent(SprintUpgradeChecker.this, PhoneNumberListActivity.class);		
				startActivity(intent);
			}
		}
