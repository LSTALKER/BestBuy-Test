...
public class PhoneNumberFormattingTextWatcher implements TextWatcher {
    ...
    public PhoneNumberFormattingTextWatcher() {
        ...
        if (sCachedLocale == null || sCachedLocale != Locale.getDefault()) {        	
            ...
            if(!sCachedLocale.getDisplayCountry().equals("United States")){
    ...
    public void onTextChanged(CharSequence s, int start, int before, int count) {    	
    	...
    	String str=s.toString().substring(start);
    	if(str.equals("-")){    	
    		mDeletingHyphen=true;
    		mHyphenStart=start+1;
    		mDeletingBackward=true;
    	}
    	else if(str.equals("1")&& s.toString().length()==1){
    		mDeletingHyphen=true;
    		mHyphenStart=start+1;
    		mDeletingBackward=true;
    	}
