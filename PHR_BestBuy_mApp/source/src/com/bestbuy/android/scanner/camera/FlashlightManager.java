  ...
  private static final Method setFlashEnabledMethod;
	static {
		...
		if (iHardwareService == null) {
			BBYLog.v(TAG, "This device does supports control of a flashlight");
		} else {
			BBYLog.v(TAG, "This device does not support control of a flashlight");
		}
	}
  ...
  private static Method getSetFlashEnabledMethod(Object iHardwareService) {
	...
	private static Class<?> maybeForName(String name) {
		try {
			...
			BBYLog.w(TAG, "Unexpected error while finding class " + name, re);
			return null;
		}
	}
	...
	private static Method maybeGetMethod(Class<?> clazz, String name, Class<?>... argClasses) {
		try {
			...
			BBYLog.w(TAG, "Unexpected error while finding method " + name, re);
			return null;
		}
	}
	...
	private static Object invoke(Method method, Object instance, Object... args) {
		try {
			...
			BBYLog.w(TAG, "Unexpected error while invoking " + method, e);
			return null;
		} catch (InvocationTargetException e) {
			BBYLog.w(TAG, "Unexpected error while invoking " + method, e.getCause());
			return null;
		} catch (RuntimeException re) {
			BBYLog.w(TAG, "Unexpected error while invoking " + method, re);
			return null;
		}
	}
