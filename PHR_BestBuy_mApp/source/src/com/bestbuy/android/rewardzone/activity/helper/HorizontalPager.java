package com.bestbuy.android.rewardzone.activity.helper;
...
public class HorizontalPager extends ViewGroup {
	...
	private PageFlickListener pageFlickListener;
	...
	public HorizontalPager(Context context, AttributeSet attrs, int defStyle) {
		...
		TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.com_rz_widget_HorizontalPager);
		pageWidthSpec = a.getDimensionPixelSize(
				R.styleable.com_rz_widget_HorizontalPager_pageWidth,
				SPEC_UNDEFINED);
		a.recycle();
		init();
	...
	public void setCurrentPage(int currentPage) {
		...
		pageFlickListener.currentPage(currentPage+1);
		invalidate();
	}
	...
	public void snapToPage(int whichPage) {
		...
		if((whichPage == getChildCount()-1 && mCurrentPage == 0) || (whichPage == 0 && mCurrentPage == getChildCount()-1))
			setCurrentPage(whichPage);
		else{
			mScroller.startScroll(getScrollX(), 0, delta, 0, Math.abs(delta) * 2);
			invalidate();
		}
		pageFlickListener.currentPage(whichPage+1);
	...
	public static interface PageFlickListener {
		public void currentPage(int page);
	}

	public void setOnPageFlickListener(PageFlickListener p) {
		pageFlickListener = p;
	}
}
